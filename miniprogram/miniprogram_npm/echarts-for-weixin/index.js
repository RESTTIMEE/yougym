...
Component({
  ...
  ready: function ready() {
    if (!this.data.echarts) {
      this.data.echarts = (typeof globalThis !== 'undefined' && globalThis.echarts) || null;
    }
    if (!this.data.echarts) {
  ...
  methods: {
    init: function init(callback, echartsLib) {
      ...
      if (isUseNewCanvas) {
        ...
        this.initByNewWay(callback, echartsLib);
      } else {
        ...
        if (!isValid) {
          ...
          this.initByOldWay(callback, echartsLib);
        }
      }
    },

    initByOldWay: function initByOldWay(callback, echartsLib) {
      var _ec = echartsLib || this.data.echarts;
      if (!_ec) return;
      var _this = this;
    ...
    initByNewWay: function initByNewWay(callback, echartsLib) {
      var _ec = echartsLib || this.data.echarts;
      if (!_ec) return;
      var _this2 = this;
...
var WxCanvas = function () {
  ...
  WxCanvas.prototype.addEventListener = function addEventListener(type, handler) {
    this._eventListeners = this._eventListeners || {};
    (this._eventListeners[type] = this._eventListeners[type] || []).push(handler);
  };

  WxCanvas.prototype.removeEventListener = function removeEventListener(type, handler) {
    if (!this._eventListeners || !this._eventListeners[type]) return;
    var idx = this._eventListeners[type].indexOf(handler);
    if (idx >= 0) this._eventListeners[type].splice(idx, 1);
  };
